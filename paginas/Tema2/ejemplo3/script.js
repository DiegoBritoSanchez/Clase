...
function potencia() {
  ...
  input.innerHTML +=
    "<h4>Introduce el exponente:</h4><input type='text' id='exp'>";
  ...
  function resultado() {
    if (parseInt(base.value) && parseInt(exp.value)) {
      if (!isNaN(base.value) && !isNaN(exp.value)) {
        res.innerHTML =
          "<tr><td>La potencia de " +
          base.value +
          " elevado a " +
          exp.value +
          " es: " +
          Math.pow(base.value, exp.value) +
          "</td></td>";
      } else {
...
function redondeo() {
  ...
  //Le asignamos un input para trabajar
  input.innerHTML =
    "<h4>Introduce un número decimal:</h4><input type='text' id='base'>";
  //Creamos el evento de escucha
  let base = document.getElementById("base");
  base.addEventListener("input", resultado);
  //Creamos la funcion del cálculo de raíz
...
function trigonometria() {
  ...
  //Le asignamos un input para trabajar
  input.innerHTML =
    "<h4>Introduce un ángulo:</h4><input type='text' id='base'>";
  //Creamos el evento de escucha
  let base = document.getElementById("base");
  base.addEventListener("input", resultado);
  //Creamos la funcion del cálculo de raíz
    ...
    if (flotante >= 0 && flotante <= 360) {
      if (!isNaN(flotante)) {
        res.innerHTML =
          ...
          flotante +
          " es " +
          Math.sin(flotante).toFixed(5) +
          "</td></tr>";
        res.innerHTML +=
          "<tr><td>El coseno de " +
          flotante +
          " es " +
          Math.cos(flotante).toFixed(5) +
          "</td></tr>";
        res.innerHTML +=
          "<tr><td>La tangente de " +
          flotante +
          " es " +
          Math.tan(flotante).toFixed(5) +
          "</td></tr>";
      } else {
        res.innerHTML = "Introduce un valor válido";
      }
